...
def base_parser():
    ...
    # ============ Model configuration =============
    parser.add_argument("--method", type=str, default="flyprompt", help="Method name", choices=METHODS.keys())
    parser.add_argument("--backbone", type=str, default="vit_base_patch16_224", help="Backbone name")

    # =========== Dataset configuration ============
    parser.add_argument("--dataset", type=str, default="cifar100", help="dataset name", choices=DATASETS.keys())
    ...
    parser.add_argument("--rnd_NM", action='store_true', default=True, help="if True, N and M are randomly mixed over tasks.")
    parser.add_argument("--no_rnd_NM", action='store_false', dest='rnd_NM',
                        help="Disable random N/M mixing and use evenly split task construction.")
    ...
    parser.add_argument("--online_iter", type=float, default=3, help="number of model updates per samples seen.")

    parser.add_argument("--transforms", nargs="*", default=[], help="Additional train transforms [cutmix, cutout, autoaug]")
    ...
    parser.add_argument("--eval_period", type=int, default=1000, help="evaluation period for true online setup")
    parser.add_argument("--eval_ema_only", action="store_true", default=False,
                        help="During evaluation, drop the online head and ensemble EMA heads only.")
    # ======== Router / Oracle analysis options =========
    parser.add_argument("--router_mode", type=str,
                        choices=["learned", "random", "single", "sample_oracle", "class_oracle", "worst"],
                        default="learned",
                        help="Router mode to use during evaluation / analysis.")
    parser.add_argument("--log_router_trace", action="store_true", default=False,
                        help="If set, save per-sample router trace during evaluation to results/logs/...")
    parser.add_argument("--analysis_router_quality", action="store_true", default=False,
                        help="If set, compute router quality metrics (RQI, NMI, entropy, etc.) and save summaries.")
    parser.add_argument("--oracle_batch_size", type=int, default=-1,
                        help="Batch size to use when enumerating routes for oracles. -1 uses eval batch size.")
    parser.add_argument("--oracle_max_routes", type=int, default=-1,
                        help="If >0, only enumerate the first K routes for sample_oracle/class_oracle to save memory.")
    ...
    parser.add_argument("--len_prompt", type=int, default=5, help="The length of the prompt for each expert")
