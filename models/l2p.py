...
class L2P(nn.Module):
    def __init__(self,
        ...
        if hasattr(vit, backbone_name):
            ...
            self.add_module('backbone', getattr(vit, backbone_name)(pretrained=False, num_classes=num_classes))
        else:
            logger.info(f'Using timm model: {backbone_name}')
            self.add_module('backbone', timm.create_model(backbone_name, pretrained=False, num_classes=num_classes))
